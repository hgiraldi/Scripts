...
(function (root) {
  ...
  function generate(data) {
    ...
    // AVISO: nenhum código lido automaticamente (ex.: QR/barcode em COR ESPECIAL/spot que
    // renderiza chapado e nao decodifica) -> pede conferencia MANUAL.
    if (data.avisoCodigo) {
      doc.setFillColor(254, 249, 231); doc.rect(M, y - 3, PW - 2 * M, 8, "F");
      doc.setFillColor(230, 164, 23); doc.rect(M + 2, y - 0.8, 2.6, 2.6, "F");
      doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(150, 100, 10);
      doc.text("Codigo (QR / barras): NAO foi possivel verificar automaticamente - CONFERIR MANUALMENTE (pode estar em cor especial).", M + 7, y + 1.2);
      y += 9;
    }
    ...
    // (As IGNORADAS não entram nesta folha — vão num relatório separado só visual.)
  ...
  // Sem textos de posição/tamanho — só as montagens (Original | Arquivo | Diferença) do que
  // o operador tirou da conferência, p/ visualização interna.
  function generateIgnored(data) {
    var jsPDFctor = (root.jspdf && root.jspdf.jsPDF) ? root.jspdf.jsPDF : null;
    if (!jsPDFctor) return { ok: false, msg: "jsPDF nao carregou" };
    var doc = new jsPDFctor({ unit: "mm", format: "a4", compress: true });
    var PW = 210, PH = 297, M = 14, y;

    doc.setFillColor(NAVY[0], NAVY[1], NAVY[2]); doc.rect(0, 0, PW, 26, "F");
    if (data.logo) { var la = data.logoAspect || 2, lh = 15, lw = lh * la; if (lw > 46) { lw = 46; lh = lw / la; }
      try { doc.addImage(data.logo, "PNG", M, 4 + (18 - lh) / 2, lw, lh); } catch (e) {} }
    doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold"); doc.setFontSize(15);
    doc.text("IGNORADAS - VISUALIZACAO INTERNA", PW - M, 13, { align: "right" });
    doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(200, 210, 225);
    doc.text("Alpha Clicheria  -  Alpha Compare", PW - M, 19, { align: "right" });

    y = 34;
    doc.setTextColor(30, 39, 53); doc.setFontSize(10);
    doc.setFont("helvetica", "bold"); doc.text("O.S.:", M, y); doc.setFont("helvetica", "normal"); doc.text(String(data.os || "-"), M + 14, y);
    doc.setFont("helvetica", "bold"); doc.text("Data:", PW / 2, y); doc.setFont("helvetica", "normal"); doc.text(String(data.dateStr || "-"), PW / 2 + 14, y);
    y += 6;
    doc.setFont("helvetica", "bold"); doc.text("Produto:", M, y); doc.setFont("helvetica", "normal"); doc.text(String(data.job || "-"), M + 20, y);
    y += 5;
    doc.setTextColor(GREYT[0], GREYT[1], GREYT[2]); doc.setFontSize(8.5);
    doc.text("Itens que o operador tirou da conferencia (nao contam como divergencia).", M, y);
    y += 6;
    ...
    for (var i = 0; i < shots.length; i++) {
      var s = shots[i], asp = s.aspect || 4.7;
      var mw = iw, ih = iw / asp; if (ih > 54) { ih = 54; mw = ih * asp; }
      var mx = M + (iw - mw) / 2, blocoH = 5 + ih + 6;
      if (y + blocoH > PH - 12) { doc.addPage(); y = M; }
      doc.setFillColor(150, 158, 168); doc.rect(M, y, 3, 3, "F");
      doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(80, 90, 105);
      var cap = "#" + (i + 1) + "  " + (s.desc || "");
      if (s.motivo) cap += "   (" + s.motivo + ")";
      doc.text(cap.length > 95 ? cap.slice(0, 93) + "…" : cap, M + 5, y + 2.6);
      y += 5;
      if (s.img) { try { doc.addImage(s.img, "PNG", mx, y, mw, ih); } catch (e) {} }
      doc.setDrawColor(210, 216, 224); doc.setLineWidth(0.2); doc.rect(mx, y, mw, ih);
      y += ih + 6;
    ...
    for (var p = 1; p <= pages; p++) { doc.setPage(p);
      doc.setFontSize(7.5); doc.setTextColor(160, 168, 178);
      doc.text("Alpha Compare - ignoradas (uso interno)  -  " + data.dateStr, M, PH - 8);
      doc.text(p + "/" + pages, PW - M, PH - 8, { align: "right" });
    }

    try {
      var fs = require("fs"), path = require("path");
      var fname = "Conferencia_" + safeName(data.os || "OS") + "_" + safeName(data.stamp || "") + "_ignoradas.pdf";
      var buf = Buffer.from(doc.output("arraybuffer"));
      if (data.reportDir) {
        try { if (!fs.existsSync(data.reportDir)) fs.mkdirSync(data.reportDir, { recursive: true });
          var full = path.join(data.reportDir, fname); fs.writeFileSync(full, buf); return { ok: true, path: full, where: "reference" };
        } catch (eR) {}
      }
      var fp = path.join(data.desktopPath, fname); fs.writeFileSync(fp, buf); return { ok: true, path: fp, where: "Desktop" };
    } catch (e) { return { ok: false, msg: String(e) }; }
  }

  root.ACReport = { generate: generate, generateIgnored: generateIgnored };
